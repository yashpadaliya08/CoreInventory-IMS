...
<div class="fade-in">
    <div class="page-header">
        <div>
            <h2 class="m-0" style="font-family: 'Outfit'; font-weight: 700; font-size: 2rem; color: var(--text-main); letter-spacing: -0.5px;">Receipts</h2>
            <p class="text-muted m-0 mt-1" style="font-size: 1rem;">Manage incoming stock from vendors.</p>
        </div>
        @if(auth()->user() && auth()->user()->isManagerOrAbove())
        <a href="{{ route('receipts.create') }}" class="btn btn-primary d-flex align-items-center gap-2" style="height: 44px; padding: 0 24px;">
            <i data-feather="plus" style="width: 18px;"></i> Create Receipt
        </a>
        @endif
    </div>
    ...
    <div class="filter-panel">
        <form method="GET" action="{{ route('receipts.index') }}" class="row g-3 align-items-end">
            ...
            <div class="col-md-4">
                <label class="form-label text-muted" style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px;">Vendor Name</label>
                <div class="position-relative">
                    <i data-feather="briefcase" style="position: absolute; top: 12px; left: 14px; color: var(--text-muted); width: 18px;"></i>
                    <input type="text" name="vendor_name" class="form-control" style="padding-left: 42px; height: 44px; background: rgba(255,255,255,0.9);" placeholder="Search vendor..." value="{{ request('vendor_name') }}">
                </div>
            </div>
            <div class="col-md-3">
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody style="border-top: 2px solid rgba(0,0,0,0.05);">
                    ...
                    <tr>
                        <td class="ps-4"><a href="{{ route('receipts.show', $receipt->id) }}" class="ref-link">{{ $receipt->reference_no }}</a></td>
                        <td>
                            <div class="fw-semibold" style="color: var(--text-main);">{{ $receipt->vendor_name ?? 'N/A' }}</div>
                        </td>
                        <td style="font-family: 'Outfit'; font-weight: 500; color: var(--text-muted);">
                            {{ $receipt->expected_date ? \Carbon\Carbon::parse($receipt->expected_date)->format('M d, Y') : 'N/A' }}
                        </td>
                        <td>
                            <span class="status-badge badge-{{ $receipt->status }}">{{ $receipt->status }}</span>
                        </td>
                        <td class="text-end pe-4">
                            <a href="{{ route('receipts.show', $receipt->id) }}" class="btn btn-sm btn-light" style="border: 1px solid rgba(0,0,0,0.1); font-weight: 600;">Open</a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="text-center py-5">
                            ...
                            <h5 style="font-family: 'Outfit'; font-weight: 600; color: var(--text-main);">No Receipts Found</h5>
