...
<div class="fade-in">
    <div class="page-header">
        <div>
            <h2 class="m-0" style="font-family: 'Outfit'; font-weight: 700; font-size: 2rem; color: var(--text-main); letter-spacing: -0.5px;">Delivery Orders</h2>
            <p class="text-muted m-0 mt-1" style="font-size: 1rem;">Manage outbound stock directed to customers.</p>
        </div>
        @if(auth()->user() && auth()->user()->isManagerOrAbove())
        <a href="{{ route('deliveries.create') }}" class="btn btn-primary d-flex align-items-center gap-2" style="height: 44px; padding: 0 24px;">
            <i data-feather="plus" style="width: 18px;"></i> Create Delivery
        </a>
        @endif
    </div>
    ...
    <div class="filter-panel">
        <form method="GET" action="{{ route('deliveries.index') }}" class="row g-3 align-items-end">
            ...
            <div class="col-md-4">
                <label class="form-label text-muted" style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px;">Customer Name</label>
                <div class="position-relative">
                    <i data-feather="users" style="position: absolute; top: 12px; left: 14px; color: var(--text-muted); width: 18px;"></i>
                    <input type="text" name="customer_name" class="form-control" style="padding-left: 42px; height: 44px; background: rgba(255,255,255,0.9);" placeholder="Search customer..." value="{{ request('customer_name') }}">
                </div>
            </div>
            <div class="col-md-3">
            ...
            <table class="table table-hover mb-0 align-middle">
                <thead style="background: rgba(0,0,0,0.02);">
                    <tr>
                        ...
                        <th class="border-0" style="color: var(--text-muted); font-size: 0.75rem; font-weight: 700; letter-spacing: 1px;">Customer</th>
                        <th class="border-0" style="color: var(--text-muted); font-size: 0.75rem; font-weight: 700; letter-spacing: 1px;">Scheduled Date</th>
                ...
                <tbody style="border-top: 2px solid rgba(0,0,0,0.05);">
                    ...
                    <tr>
                        <td class="ps-4"><a href="{{ route('deliveries.show', $delivery->id) }}" class="ref-link">{{ $delivery->reference_no }}</a></td>
                        <td>
                            <div class="fw-semibold" style="color: var(--text-main);">{{ $delivery->customer_name ?? 'N/A' }}</div>
                        </td>
                        <td style="font-family: 'Outfit'; font-weight: 500; color: var(--text-muted);">
                            {{ $delivery->scheduled_date ? \Carbon\Carbon::parse($delivery->scheduled_date)->format('M d, Y') : 'N/A' }}
                        </td>
                        <td>
                            <span class="status-badge badge-{{ $delivery->status }}">{{ $delivery->status }}</span>
                        </td>
                        <td class="text-end pe-4">
                            <a href="{{ route('deliveries.show', $delivery->id) }}" class="btn btn-sm btn-light" style="border: 1px solid rgba(0,0,0,0.1); font-weight: 600;">Open</a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="text-center py-5">
                            <i data-feather="truck" style="width: 48px; height: 48px; color: var(--text-muted); opacity: 0.5; margin-bottom: 16px;"></i>
                            <h5 style="font-family: 'Outfit'; font-weight: 600; color: var(--text-main);">No Deliveries Found</h5>
