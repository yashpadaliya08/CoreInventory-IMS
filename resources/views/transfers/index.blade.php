...
<div class="fade-in">
    <div class="page-header">
        <div>
            <h2 class="m-0" style="font-family: 'Outfit'; font-weight: 700; font-size: 2rem; color: var(--text-main); letter-spacing: -0.5px;">Internal Transfers</h2>
            <p class="text-muted m-0 mt-1" style="font-size: 1rem;">Move stock between your internal zones and hubs.</p>
        </div>
        @if(auth()->user() && auth()->user()->isManagerOrAbove())
        <a href="{{ route('transfers.create') }}" class="btn btn-primary d-flex align-items-center gap-2" style="height: 44px; padding: 0 24px;">
            <i data-feather="plus" style="width: 18px;"></i> New Transfer
        </a>
        @endif
    </div>

    <div class="filter-panel">
        <form method="GET" action="{{ route('transfers.index') }}" class="row g-3 align-items-end">
            <div class="col-md-7">
            ...
            <table class="table table-hover mb-0 align-middle">
                <thead style="background: rgba(0,0,0,0.02);">
                    <tr>
                        ...
                        <th class="border-0" style="color: var(--text-muted); font-size: 0.75rem; font-weight: 700; letter-spacing: 1px;">Source Location</th>
                        <th class="border-0" style="color: var(--text-muted); font-size: 0.75rem; font-weight: 700; letter-spacing: 1px;">Destination Location</th>
                ...
                <tbody style="border-top: 2px solid rgba(0,0,0,0.05);">
                    ...
                    <tr>
                        <td class="ps-4"><a href="{{ route('transfers.show', $transfer->id) }}" class="ref-link">{{ $transfer->reference_no }}</a></td>
                        <td><span class="loc-tag text-danger"><i data-feather="minus-circle" style="width: 12px; margin-right:4px;"></i>{{ $transfer->fromLocation->name ?? 'N/A' }}</span></td>
                        <td><span class="loc-tag text-success"><i data-feather="plus-circle" style="width: 12px; margin-right:4px;"></i>{{ $transfer->toLocation->name ?? 'N/A' }}</span></td>
                        <td>
                            <span class="status-badge badge-{{ $transfer->status }}">{{ $transfer->status }}</span>
                        </td>
                        <td class="text-end pe-4">
                            <a href="{{ route('transfers.show', $transfer->id) }}" class="btn btn-sm btn-light" style="border: 1px solid rgba(0,0,0,0.1); font-weight: 600;">Open</a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="text-center py-5">
                            <i data-feather="repeat" style="width: 48px; height: 48px; color: var(--text-muted); opacity: 0.5; margin-bottom: 16px;"></i>
                            <h5 style="font-family: 'Outfit'; font-weight: 600; color: var(--text-main);">No Transfers Found</h5>
